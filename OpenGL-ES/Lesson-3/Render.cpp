...
void Render::render1(egl::Context &context)
{
	if (m_programCache.empty()) {
		...
		GLbyte fShaderSrc[] =
			...
			"}";
		m_programCache.push_back(gles::Program((char*)vShaderSrc, (char*)fShaderSrc));
		m_programCache[0].compile();
		m_programCache[0].link();
	}

	gles::Program &program = m_programCache[0];
