...
bool TestEGLError(HWND nativeWindow, const char* functionLastCalled)
{
	...
	*/
	...
	if (lastError != EGL_SUCCESS)
	{
		...
		_stprintf_s(stringBuffer, 256, _T("%s failed (%x).\n"), functionLastCalled, lastError);
...
bool InitialiseShaders( GLuint &fragmentShader, GLuint &vertexShader, GLuint &shaderProgram, HWND nativeWindow ) 
{
	...
	*/
	...
	if (!isShaderCompiled)
	{
		...
		TCHAR* wInfoLog = new TCHAR[infoLogLength];
		glGetShaderInfoLog(fragmentShader, infoLogLength, &charactersWritten, infoLog);
		MultiByteToWideChar(CP_ACP, 0, infoLog, infoLogLength, wInfoLog, infoLogLength);

		// Display the error in a dialog box
		MessageBox(nativeWindow, infoLogLength>1 ? wInfoLog : _T("Failed to compile fragment shader. (No information)"),
	...
	*/
	...
	if (!isShaderCompiled)
	{
		...
		// Display the error in a dialog box
		MessageBox(nativeWindow, infoLogLength>1 ? wInfoLog : _T("Failed to compile vertex shader. (No information)"), 
		           ERROR_TITLE, MB_OK | MB_ICONEXCLAMATION);
	...
	if (!isLinked)
	{
		...
		// Display the error in a dialog box
		MessageBox(nativeWindow, infoLogLength>1 ? wInfoLog : _T("Failed to link GL program object. (No information)"), 
		           ERROR_TITLE, MB_OK | MB_ICONEXCLAMATION);
...
int WINAPI WinMain( _In_ HINSTANCE applicationInstance, _In_opt_ HINSTANCE previousInstance, _In_ LPSTR /*lpCmdLine*/, _In_ int /*nShowCmd*/ )
{
	// Windows variables
